...
interface IEdit {
  selectedCourses: ICourseDB | null,
  editCloseModal: () => void,
...
const CourseEditModal: React.FC<IEdit> = ({ selectedCourses, editCloseModal, sidebarCloseModal }) => {
  ...
  const [editFormData, setEditFormData] = useState<ICourseEditModal>({
    ...
    courseDescription: selectedCourses?.courseDescription || '',
    coursePrice: selectedCourses?.coursePrice || '',
    courseDuration: selectedCourses?.courseDuration || '',
    courseLevel: selectedCourses?.courseLevel || 'beginner',
    courseThumbnail: selectedCourses?.courseThumbnail || '', // File input, leave empty
    categoryId: selectedCourses?.category_id || '',
    teacherId: selectedCourses?.teacher_id || ''
  });
  const [Loading, setLoading] = useState(false);
  const { data: categories } = useAppSelector((store) => store.category);
  const { data: teachers } = useAppSelector((store) => store.teacher);
  ...
  return (
    <div id="modal" className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-lg shadow-xl max-h-[95vh] overflow-y-auto">
        ...
        <form onSubmit={handleEditFormSubmission} className="px-6 py-4">
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  onChange={handleChange}
                  name="courseLevel"
                  value={editFormData?.courseLevel}
            ...
            {selectedCourses?.courseThumbnail && !editFormData.courseThumbnail && (
              <div className="mt-2">
                <p className="text-xs text-gray-500 mb-1">Current thumbnail:</p>
                <img
                  src={selectedCourses?.courseThumbnail}
                  alt="Current"
